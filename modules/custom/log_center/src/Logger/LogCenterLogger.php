...
namespace Drupal\log_center\Logger;

use Drupal\log_center\Service\LogCenterWriter;
...
final class LogCenterLogger implements LoggerInterface {
  ...
  public function log($level, string|\Stringable $message, array $context = []): void {
    ...
    // Avoid recursion if we log from log_center channel writes that error.
    $channel = (string) ($context['channel'] ?? ($context['@channel'] ?? 'php'));
    if ($channel === 'log_center') {
      return;
    }
