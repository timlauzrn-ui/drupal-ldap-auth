#!/usr/bin/env bash
# Verify ldap_role_mapper mapping/overwrite logic and module status inside my-drupal.
set -euo pipefail
...
docker exec -u www-data -w /opt/drupal "${CONTAINER}" vendor/bin/drush cget ldap_role_mapper.settings
docker exec -u www-data -w /opt/drupal "${CONTAINER}" vendor/bin/drush php:eval "echo \\Drupal\\Core\\Url::fromRoute('ldap_role_mapper.settings')->toString(), PHP_EOL;"
...
if (!\$mh->hasImplementations('user_login', 'ldap_role_mapper')) {
  throw new \\Exception('ldap_role_mapper_user_login not registered');
}
\$mods = [];
...
docker exec -u www-data -w /opt/drupal "${CONTAINER}" vendor/bin/drush php:eval "
\$sync = \\Drupal::service('ldap_role_mapper.synchronizer');
